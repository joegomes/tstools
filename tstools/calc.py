...
class TMDRunner(object):

    def __init__(self, atoms, charge=0, mult=1, nt=12):

        assert charge == 0 and mult == 1
        #checkpoint = "params/epoch=359-val_loss=0.0212-test_loss=0.2853.ckpt"
        #checkpoint = "params/finetuned.ckpt"
        checkpoint = "params/pretrained.ckpt"
        ...
        self.z = torch.from_numpy(self.numbers).long()
        self.batch = torch.zeros(len(self.numbers), dtype=torch.long)
        self.pos = None
    ...
    def energy(self, positions):
        ...
        _ = self.write_xyz(positions)
        energy, _ = self.model(self.z, self.pos, self.batch)
    ...
    def grad(self, positions):
        ...
        _ = self.write_xyz(positions)
        energy, grad = self.model(self.z, self.pos, self.batch)
        # convert energy from eV to Hartree
        energy = energy.item() / Hartree
        # convert grad from eV/Angs to Hartree/Angs
        grad = -grad.detach().numpy().flatten() / Hartree
        return energy, grad
